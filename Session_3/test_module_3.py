...
@pytest.mark.parametrize(
    ...
    [
        pytest.param(
            ...
            [0, 2, 3, 4],
            [0, 8, 27, 64],
            id="Example: exp 3, base [0, 1, 2, 3]"
        ),
        pytest.param(
            0,
            [0, 1, 5, 10],
            [1, 1, 1, 1],
            id="Example: exp 0, base [0, 1, 5, 10]"
        ),
        pytest.param(
            -1,
            [1, 5, 10, -1, -5, -10],
...
def test_task_1_correct_work(exp, bases, expected):
    ...
    for idx in range(len(bases)):
        ...
        assert out == expected[idx], (
            "Incorrect power operation result."
        )
...
def test_task_2_correct_print(capsys):
    ...
    assert expected in out, (
        f"Positional argument {1} is missed"
    )


def test_task_3_correct_decorator_print(capsys):
    task_3("Egor")
    expected = "Hi, friend! What's your name?\n" \
...
def test_task_5_correct_work(size):
    ...
    assert np.allclose(transposed_matrix_np, transposed_matrix_task), (
...
def test_task_6_correct_work(brackets, expected):
    ...
    assert is_correct_brackets == expected, (
        "Incorrect answer. Check tests!"
    )
